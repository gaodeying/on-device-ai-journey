...
> **学习周期**：2026-04-01 → 2026-06-23（12周）
> **制定时间**：2026年3月25日 | **版本**：v3.0（根据专业评估优化版）
> **可行性评分**：85分（可行，需合理管理预期）
...
休息机制：
  每3周安排1个"轻度周末"（只做复习，不学新内容）
  遇到卡壳超过2天，立即降级到"了解级"，不要死磕
```

---

### 第1阶段：重塑 AI 技术栈基础（W1-W4）

**阶段目标**：具备用 Python + PyTorch 加载/导出模型的基本能力，重激活现代 C++，理解 AI 关键架构

**调整说明**：原计划W1-W3压缩为3周，现扩展为4周，增加基础巩固时间，提升容错率。

---

#### W1（4/1-4/6）：Python + PyTorch 基础强化

> **核心目标**：建立张量思维，掌握 Numpy/PyTorch 核心操作（移动开发者重点突破）

| 天 | 日期 | 内容 | 具体任务 |
|----|------|------|---------|
| 周三 | 4/1 | 环境搭建 | 安装 Conda、PyTorch；配置 venv；确认 Python 3.10+；跑通 `torch.randn(2,3)` |
| 周四 | 4/2 | Python基础扫盲 | List/Dict/Set推导式、装饰器、Context Manager（`with`语句） |
| 周五 | 4/3 | Numpy实战 | 数组广播机制、切片操作、矩阵运算（dot/transpose） |
| 周六 | 4/4 | PyTorch Tensor | Tensor与Numpy互转、设备迁移（`.to('cuda')`）、自动求导（`.backward()`） |
| 周日 | 4/5 | 预训练模型加载 | `torchvision.models.resnet50(weights='DEFAULT')` 跑推理，打印层结构 |
| 周一 | 4/6 | 复盘 + 写笔记 | 整理"移动开发者学习PyTorch第1周"心路历程 |

**本周产出**：PyTorch 基础 Demo + 第一篇技术博客

**学习资源**：
- [PyTorch 60分钟入门](https://pytorch.org/tutorials/beginner/deep_learning_60min_blitz.html)
- [Python 官方教程 - 推导式](https://docs.python.org/3/tutorial/datastructures.html#list-comprehensions)
- [Numpy 速查表](https://numpy.org/doc/stable/user/quickstart.html)
...
| 天 | 日期 | 内容 | 具体任务 |
|----|------|------|---------|
| 周二 | 4/7 | ONNX 概念理解 | 为什么需要ONNX（中间格式、跨框架、端侧标准） |
| 周三 | 4/8 | ONNX 导出 | `torch.onnx.export()` 导出 ResNet50，理解 `dynamic_axes` 参数 |
| 周四 | 4/9 | Netron 可视化 | 用 Netron 打开 `.onnx` 文件，理解节点/边/输入输出 |
| 周五 | 4/10 | ONNX Runtime Python | 安装 `onnxruntime`，跑推理，对比 PyTorch 输出 |
| 周六 | 4/11 | 图优化入门 | 用 `onnx-simplifier` 做图折叠，观察节点数变化 |
| 周日 | 4/12 | 端到端Demo | 完整脚本：加载模型 → 导出ONNX → 推理 → 打印Top5结果 |
| 周一 | 4/13 | 博客发布 | "移动开发者第一份ONNX模型导出实战" |

**本周产出**：完整的 Python 推理脚本 + 第二篇技术博客
...
> **重点**：移动开发者重点掌握智能指针、move语义，其他查用即可

| 天 | 日期 | 内容 | 具体任务 |
|----|------|------|---------|
| 周二 | 4/14 | C++11/14/17 现代特性 | `unique_ptr`、`shared_ptr`、`auto`、`lambda`、`move` 语义 |
| 周三 | 4/15 | 内存对齐与高效分配 | `std::vector` 内存布局、`aligned_alloc`（AI 推理核心基础） |
| 周四 | 4/16 | ONNX Runtime C++ API | 加载 `.onnx` 文件，设置 `SessionOptions`，跑一次推理 |
| 周五 | 4/17 | 前后处理 C++ 实现 | 图片解码用 **stb_image**（单头文件，工程常用），写 resize/normalize |
| 周六 | 4/18 | 异常处理与性能计时 | `std::chrono` 计时，try-catch 封装推理错误 |
| 周日 | 4/19 | 封装 CLI 工具 | 做一个命令行：`./infer --model xxx.onnx --image xxx.jpg` |
| 周一 | 4/20 | JNI 接口设计 | 设计 `InferenceEngine` C++ 类并封装 JNI 接口，为下周 Android 做准备 |
...
```
✅ 必学：
- unique_ptr/shared_ptr（端侧AI推理资源管理核心）
...
- [cppreference.com（快速查阅）](https://en.cppreference.com/w/)
...
| 天 | 日期 | 内容 | 具体任务 |
|----|------|------|---------|
| 周二 | 4/21 | Android 环境准备 | 配置 NDK r25+，下载 `onnxruntime-android` AAR |
| 周三 | 4/22 | JNI/NDK 接入 ORT | CMake 集成 ONNX Runtime，Java 层调 C++ 推理接口 |
| 周四 | 4/23 | CameraX 实时推理 | 接入相机帧流，做帧采样 + 推理 |
| 周五 | 4/24 | 线程与内存优化 | 推理放 worker thread，避免 ANR；监控内存泄漏 |
| 周六 | 4/25 | 打点统计 | FPS、单帧推理延迟、内存 RSS 打印到 logcat |
| 周日 | 4/26 | UI 展示 + 整理性能报告 | 在画面上叠加推理结果，整理性能表格 |
| 周一 | 4/27 | 录屏 + 提交 GitHub | 推 README + demo.gif + 性能数据表 |
...
- [ONNX Runtime Android Sample](https://github.com/microsoft/onnxruntime-inference-examples/tree/main/mobile/examples/android)
...
> **风险预判**：LLM量化对本地开发环境要求高（内存、GPU），准备B计划

| 天 | 日期 | 内容 | 具体任务 |
|----|------|------|---------|
| 周二 | 4/28 | 量化理论 | PTQ vs QAT，FP32/FP16/BF16/INT8/INT4 精度与范围；剪枝/蒸馏概念了解 |
| 周三 | 4/29 | PTQ 动态量化实践 | `onnxruntime.quantization.quantize_dynamic()` 对视觉模型量化 |
| 周四 | 4/30 | PTQ 静态量化 | 校准集 + 最大值校准 vs KL 散度校准，Scale/Zero-point 计算 |
| 周五 | 5/1 | 端上实测量量模型 | 在 W4 的 Android App 中切换量化前后模型，记录延迟/内存/精度 |
| 周六 | 5/2 | LLM 量化实践 | 用 `torchao.quantize_(model, int4_weight_only())` 对小模型做 INT4 量化 |
| 周日 | 5/3 | 写量化实验报告 | 对比表：模型体积、内存峰值、延迟、精度损失 |
| 周一 | 5/4 | 面试题演练 | 口述"PTQ 和 QAT 的区别"等高频题 |
...
| 风险点 | 预判 | B计划 |
|-------|------|-------|
| Qwen2.5-1.5B 量化内存不足 | MacBook 16GB可能不够 | 改用 **Qwen2.5-0.5B** 或 **Phi-3-mini-4k** |
| 校准集准备耗时 | 需要收集1000+张图片 | 用 **ImageNet 验证集**（官方提供） |
| PTQ vs QAT概念混淆 | 面试可能被追问 | 背诵：QAT需要重训练，3个月内不动手 |
...
### 实验配置
- 模型：MobileNetV3-Large（CV） + Qwen2.5-0.5B（LLM）
- 设备：iPhone 15 Pro / Android 13真机
- 量化工具：onnxruntime.quantize（CV） + torchao（LLM）
...
| 模型体积 | 23MB | 6MB | 74% ↓ |
| 单帧延迟 | 180ms | 52ms | 71% ↓ |
| Top1精度 | 75.2% | 74.8% | 0.4% |

### 结果对比（LLM模型）
| 指标 | FP16 | INT4 | 降幅 |
|------|------|------|------|
| 模型体积 | 1.2GB | 420MB | 65% ↓ |
| 首字延迟(TTFT) | 820ms | 340ms | 59% ↓ |
| 生成速度 | 8.5 t/s | 12.3 t/s | +45% |

### 结论
- INT8量化几乎无损，LLM INT4对对话质量影响可控
- 量化是端侧部署的必选项，延迟收益远大于精度损失
```
...
> **调整说明**：本周开始进入缓冲周节奏，降低学习强度，防止疲劳

| 天 | 日期 | 内容 | 具体任务 |
|----|------|------|---------|
| 周二 | 5/5 | LiteRT 生态认知 | 了解 LiteRT 定位：多框架统一端侧运行时 |
| 周三 | 5/6 | 模型格式转换 | 用 `ai-edge-litert` converter 把 ONNX 模型转为 `.tflite` 格式 |
| 周四 | 5/7 | LiteRT Android 接入 | 新建 Demo：引入 `com.google.ai.edge.litert` 依赖，加载 `.tflite` 模型 |
| 周五 | 5/8 | GPU Delegate 实测 | 接入 `GpuDelegateFactory`，对比 CPU vs GPU 延迟 |
| 周六 | 5/9 | 官方 Codelab | 跑通官方 image segmentation codelab |
| 周日 | 5/10 | 整理框架选型对比 | 整理 ONNX Runtime vs LiteRT：接入复杂度、模型格式要求 |
| 周一 | 5/11 | 复盘 + 预习 iOS | 整理 LiteRT 接入 checklist；阅读 Core ML 官方概览 |
...
| 天 | 日期 | 内容 | 具体任务 |
|----|------|------|---------|
| 周二 | 5/12 | Core ML 框架认知 | 了解 Core ML 格式（`.mlpackage`），Apple Neural Engine 调用路径 |
| 周三 | 5/13 | coremltools 转换 | `ct.convert()` 把 PyTorch 模型转为 Core ML 格式 |
| 周四 | 5/14 | iOS 相机推理 | AVFoundation 接帧 + Core ML Vision 推理 + SwiftUI 展示 |
| 周五 | 5/15 | Instruments 性能分析 | 用 Instruments Time Profiler + Allocations 分析推理耗时和内存 |
| 周六 | 5/16 | Core ML 性能优化 | 开启 ANE（`computeUnits: .all`），对比 CPU/GPU/ANE 延迟 |
| 周日 | 5/17 | 双端性能对比表 | Android vs iOS：延迟/内存/功耗横向对比 |
| 周一 | 5/18 | 复盘 + 提交 | iOS Demo 推 GitHub，整理双端对比笔记 |
...
| 天 | 日期 | 内容 | 具体任务 |
|----|------|------|---------|
| 周二 | 5/19 | ExecuTorch 定位认知 | 了解 ExecuTorch 是 PyTorch Mobile 的官方替代 |
| 周三 | 5/20 | ExecuTorch 本地跑通 | 在 **macOS 桌面端**跑通 ExecuTorch Hello World |
| 周四 | 5/21 | ncnn 认知与接入 | 了解 ncnn（腾讯开源）；跑通 ncnn Android demo |
| 周五 | 5/22 | 四框架生态对比 | 社区活跃度、模型支持范围、C++ API 易用性 |
| 周六 | 5/23 | 统一抽象层设计 | 思考：跨框架推理 SDK 如何抽象 |
| 周日 | 5/24 | 写对比报告 | 四框架对比表：适用场景、优劣势、框架状态 |
| 周一 | 5/25 | **简历初稿** | 写简历初稿，准备"如何选推理框架"的完整回答 |
...
```
新项目选推理框架 → 跨平台需求? → 是 → ONNX Runtime
                              → 否 → Google生态? → 是 → LiteRT + GPU Delegate
                                        → 否 → Apple生态? → 是 → Core ML + ANE
                                                  → 否 → PyTorch原生导出? → 是 → ExecuTorch
                                                              → 否 → 国内项目? → 是 → ncnn
                                                                        → 否 → 回到ONNX Runtime
```
...
**面试必答题**："如果让你设计一个跨端推理SDK，你会怎么抽象？"

> **参考回答**：
> "我会做三层抽象：
> 1. **模型层**：统一用ONNX作为中间格式，屏蔽PyTorch/JAX/TensorFlow差异
> 2. **引擎层**：用工厂模式按平台选择后端——Android用ONNX Runtime/LiteRT，iOS用Core ML，桌面端用ORT
> 3. **API层**：提供统一的C接口，上层用JNI/FFI封装
>
> 这样可以做到模型一次导出，多平台运行，后期换引擎也很容易。"
...
| 天 | 日期 | 内容 | 具体任务 |
|----|------|------|---------|
| 周二 | 5/26 | ARM/GPU/NPU 架构梳理 | CPU/GPU/NPU 各自擅长的场景 |
| 周三 | 5/27 | torch.profiler 使用 | 分析模型每层算子耗时，找出 Top 3 瓶颈层 |
| 周四 | 5/28 | ONNX 图优化深挖 | 用 `onnxoptimizer` 做 Conv-BN 融合、常量折叠 |
| 周五 | 5/29 | Android Perfetto 实战 | 用 Perfetto 分析 C++ 推理调用栈，找 CPU 占用热点 |
| 周六 | 5/30 | 算子融合与 Delegate 选择 | Fused Attention、Conv-BN-ReLU 融合原理 |
| 周日 | 5/31 | 多线程推理优化 | 设置 ORT `intra_op_num_threads`，测试不同线程数 |
| 周一 | 6/1 | 写"性能优化手册" | 整理一份"我的端侧推理性能排查 SOP" |
...
- 技术栈：C++ + NDK + llama.cpp
- 模型：Qwen2.5-0.5B 做 INT4 量化（体积 <1GB）
- 功能：Android App 本地 LLM 对话，无网络，流式输出

| 天 | 日期 | 内容 | 具体任务 |
|----|------|------|---------|
| 周二 | 6/2 | 选方案 + 环境准备 | 选方案；配置依赖，下载量化模型 |
| 周三 | 6/3 | 本地推理接入 | Android 接入，验证模型可跑 |
| 周四 | 6/4 | 流式输出 | 实现 token-by-token 流式显示 |
| 周五 | 6/5 | KV Cache 理解 + 性能采集 | 理解 KV Cache 原理；记录 TTFT、tokens/s、内存峰值 |
| 周六 | 6/6 | README + Benchmark 表格 | 写 GitHub README，包含架构图 + 性能数据 |
| 周日 | 6/7 | **简历定稿** | 把两个项目（W4 视觉推理 + W10 LLM）写进简历 |
| 周一 | 6/8 | 博客发布 | 发布"如何在 Android 上跑通本地 LLM" |
...
| 天 | 日期 | 内容 | 具体任务 |
|----|------|------|---------|
| 周二 | 6/9 | 具身智能概念建立 | 了解 VLA（Vision-Language-Action）、策略网络 |
| 周三 | 6/10 | LeRobot 入门 | 阅读 HuggingFace LeRobot 文档，理解数据格式 |
| 周四 | 6/11 | ROS2 环境搭建 | 安装 ROS2 Humble，跑通 talker/listener 示例 |
| 周五 | 6/12 | ROS2 + Gazebo 仿真 | 跑通 Turtlebot3 仿真，导航一个简单场景 |
| 周六 | 6/13 | 端侧推理与机器人的连接点 | 思考：手机端侧 AI 如何迁移到 Jetson/RK3588 |
| 周日 | 6/14 | 机器人公司 JD 调研 | 调研智元/宇树/银河通用/大疆的端侧工程师 JD |
| 周一 | 6/15 | 写方向规划文章 | 整理"从手机端 AI 到具身智能机器人"技术路径 |
...
**调整说明**：简历在W8初稿、W10定稿，W12不再改简历，只做面试强化和投递

---

#### W12（6/16-6/23）：面试冲刺 + 批量投递

> **重要**：第 8 周简历初稿出来后就可以开始投递，不要等 W12。边面边学，面试反馈是最好的学习信号。

| 天 | 日期 | 内容 | 具体任务 |
|----|------|------|---------|
| 周二 | 6/16 | 项目讲解稿打磨 | 每个项目准备 3 分钟口头讲解稿 |
| 周三 | 6/17 | 高频八股强化 | 集中过"高频面试题"，能流利口述 |
| 周四 | 6/18 | 系统设计题 | 练习"如何设计跨端 AI 推理 SDK" |
| 周五 | 6/19 | LeetCode 专项冲刺 | 集中刷 10 题高频题 |
| 周六 | 6/20 | 模拟面试 | 找朋友或录视频做 Mock Interview（1.5小时） |
| 周日 | 6/21 | 定向投递 | 投递 20+ 目标岗位 |
| 周一 | 6/22 | 复盘迭代 | 根据面试反馈，补短板 |
| 周二 | 6/23 | 调整投递策略 | 继续投递 + 面试 |
...
| 资源 | 用途 | 推荐理由 |
|------|------|----------|
| [cppreference.com](https://en.cppreference.com/w/) | 快速查阅API | 比《现代C++教程》更适合查漏补缺 |
| [Google C++ Style Guide](https://google.github.io/styleguide/cppguide.html) | 代码规范 | 面试官会看你的C++代码风格 |
...
| 周次 | 简历动作 | 说明 |
|------|---------|------|
| W1 | 建立GitHub仓库 | 命名规范（如`on-device-ai-journey`），每周提交 |
| W4 | 写项目README模板 | 包含：技术栈、架构图、Benchmark表格、截图 |
| W6 | 技能关键词表整理 | 把"INT8量化"等术语整理到文档，方便CV到简历 |
| W8 | 简历初稿（开始投递） | 不追求完美，有个框架即可 |
| W10 | 简历定稿 | 结合W4、W10项目数据 |

### 简历命名建议（ATS友好）

```
张三_端侧AI工程师_GitHub链接.pdf
简历末尾附加：
- GitHub: github.com/username/on-device-ai
- 掘金: juejin.cn/user/xxx (如有博客)
- 期望岗位：端侧AI开发工程师 / AI SDK工程师
```
...
| W5 LLM 量化内存不足 | 改用 **Qwen2.5-0.5B** 或 **Phi-3-mini-4k**（更小），或直接用 llama.cpp 的预量化模型跳过量化步骤 |
...
```
Week 1  ✅ 建立GitHub仓库，开始每周提交
Week 2  ✅ Python+PyTorch+ONNX推理闭环跑通 (4/13完成)
Week 3  ✅ C++ CLI推理Demo完成 (4/20完成)
Week 4  ✅ Android实时推理App（第一个面试资格里程碑）(4/27完成)
Week 5  ✅ 量化实验报告（面试核心素材）(5/4完成)
Week 6  ✅ 开始LeetCode刷题，完成LiteRT Demo (5/11完成)
Week 7  ✅ iOS Core ML Demo，双端性能对比 (5/18完成)
Week 8  ✅ 简历初稿（开始投递），四框架对比报告 (5/25完成)
Week 9  ✅ 性能优化SOP，优化前后对比数据 (6/1完成)
Week 10 ✅ 端侧LLM Demo，简历定稿 (6/8完成)
Week 11 ✅ ROS2仿真PoC，机器人技术认知 (6/15完成)
Week 12 ✅ 批量投递，面试冲刺 (6/23完成)
```
...
| 优先级 | 建议 | 理由 |
|-------|------|------|
| **必须执行** | W1-2拆分，增加C++学习资源 | 基础不牢，后面全是空中楼阁 |
| **强烈建议** | 简历准备提前到W6 | 边学边改，面试反馈更及时 |
| **建议执行** | 量化专项加B计划 | 避免环境问题卡死进度 |
| **建议执行** | 弹性周模式 + 休息机制 | 防止疲劳，提高完成率 |
| **重要提醒** | 第8周开始投递，不等W12 | 边面边学，面试反馈是最好的学习信号 |
...
**正确的心态**：
- 不要追求100%完成——完成80%并找到工作，远比拖到3个月后还在学理论要好
- 遇到卡壳超过2天，立即降级或跳过，不要死磕
- 面试官问的不会就直说，然后引导到你擅长的领域（如移动端经验）
- 你的移动端经验是护城河，不要因为转行就全部抛弃

**常见误区**：
- ❌ 想要把所有理论都学透再动手——正确：边做边学
- ❌ 认为量化一定要从零实现——正确：用工具，理解原理即可
- ❌ 觉得3个月太短，想要延期——正确：3个月后开始工作，在工作中继续学习
- ❌ 面试时不好意思提移动端经验——正确：这是你的差异化优势

---

*制定时间：2026年3月25日*
*版本：v3.0（专业评估优化版，含日期版）*
